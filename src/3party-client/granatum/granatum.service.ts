...
import { GranatumTransactionsService } from './granatum-transactions/granatum-transactions.service';
...
export class GranatumService {
  ...
  formatInvoices(data: GroupedInvoiceComponents[]): GranatumTransactionPostDTO[] {
    return data.map((group) => {
      return this.transactions.templateTransaction(group);
    });
  }

  formatRevenues(data: GroupedInvoiceComponents[]): GranatumTransactionPostDTO[] {
    return data.map((group) => {
      return this.transactions.templateTransaction(group);
    });
  }
  formatOnlendings(data: GroupedInvoiceComponents[]): GranatumTransactionPostDTO[] {
    return data.map((group) => {
      return this.transactions.templateTransaction(group);
    });
  }

  async postTransactions(formatedData: GranatumTransactionPostDTO[]) {
    try {
      for (const post of formatedData) {
        this.transactions.postTransactions(post);
      }
    } catch (error) {
